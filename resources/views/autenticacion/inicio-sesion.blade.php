...
<body>
    ...
    <div id="overlay-bloqueo">
        <svg class="icon-bloqueo" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z">
            </path>
        </svg>
        <div class="titulo-bloqueo">ACCESO DENEGADO</div>
        <div class="subtitulo-bloqueo">Dirección IP bloqueada temporalmente debido a solicitudes excesivas (Protección
            DoS).</div>
        <div class="countdown" id="timer">00:00</div>
        <div class="footer-bloqueo">HTTP 429 DEMASIADOS INTENTOS</div>
    </div>

    <div class="login-contenedor">
        <h2>Ingresar</h2>
        <div id="mensaje-error-api" class="mensaje-error-api"></div>
        <form id="formulario-login">
            ...
            <button type="submit" id="boton-login">Iniciar Sesión</button>
        </form>
        <div class="enlaces">
            <a href="{{ url('/recuperar-contrasena') }}">¿Olvidaste tu contraseña?</a><br>
            <a href="{{ url('/registro') }}">¿No tienes cuenta? Registrarse</a>
        </div>
    </div>
    ...
    <script>
        ...
        const errorApi = document.getElementById('mensaje-error-api');

        function startCountdown(seconds) {
            const expirationTime = Date.now() + (seconds * 1000);
            localStorage.setItem('block_until', expirationTime);
            showOverlay(seconds);
        }
        ...
        document.getElementById('formulario-login').addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (response.status === 429) {
                    const seconds = result.reintento_en_segundos || 60;
                    startCountdown(seconds);
                    return;
                }

                if (response.ok) {
                    if (result.requiere_mfa || result.requiere_configurar_mfa) {
                        localStorage.setItem('temp_token', result.token_temporal);
                        localStorage.setItem('requiere_setup_mfa', result.requiere_configurar_mfa ? 'true' : 'false');
                    ...
                    errorApi.innerText = result.mensaje || 'Credenciales no coinciden con nuestros registros.';
                    errorApi.style.display = 'block';
                }
            } catch (error) {
                console.error('Error:', error);
                errorApi.innerText = 'Error de comunicación con el servidor.';
                errorApi.style.display = 'block';
            } finally {
